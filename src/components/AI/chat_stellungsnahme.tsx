...
import assistantContext from '@/config/ai/assistant_context/stellungsnahme_content_config.json'
import sidebarExamples from '@/config/ai/sidebar_examples/stellungsnahme_sidebar_config.json'
import { Message } from '@/types/ai' // Importing the Message type
...
export default function ChatStellungsnahme() {
  ...
  const { messages, input, setInput, handleInputChange, handleSubmit, isLoading, error } = useChat({
    initialMessages: assistantContext.initialMessages as Message[], // Casting to Message[]
  })
            ...
            <div className='mt-2 bg-gray-100 dark:bg-gray-800 pl-13 rounded-md max-h-[500px] overflow-y-auto'>
              ...
              <ul className='p-1 mt-1 text-sm text-zinc-700 dark:text-zinc-300'>
                {sidebarExamples.examples.map((example, index) => (
                  <li key={index} onClick={() => handleLiClick(example)}>{example}</li>
                ))}
              </ul>
            </div>
            <div className='mt-3'>
